...
const recipeIngredientsDisplay = document.querySelector('.recipe__ingredient-list');
...
const getData = async function(item) {
    try {
        ...
        if (result.length === 0) throw new Error('Invalid search Term, please try again!');
        return result;
    } catch (err) {
        alert(err);
    }
...
const renderError = function (error) {
  ...
  recipeContainer.appendChild(div);
    ...
    } else console.error('<a> has no data-id attribute');
};
